...
describe('Users endpoints', () => {
    ...
    it('user should be able to like a photo', async() => {
       // create a photo
       const photo = (await request(app).post('/photos')
       .send({ filename: 'coyotivtestingsession.png'})).body
      console.log('-------------photo--', photo)

       // create a user
      const userWithPhoto = (await request(app).post('/users')
      .send({
      ...
      await request(app).post(`/users/${userWithPhoto._id}/adds`)
      ...
      const createdLikerUser = (await request(app).post('/users')
      .send(likerUser)).body
      console.log('-------------createdLikerUser--', createdLikerUser)

      // like the photo with that another user
      await request(app).post(`/users/${createdLikerUser._id}/likes`)
       .send({ photoId: photo._id })
      
       const finalPhotoUser = (await request(app)
       .get(`/users/${userWithPhoto._id}/json`)).body
       console.log('-------------finalPhotoUser--', finalPhotoUser)

       const finalLikerUser = (await request(app)
       .get(`/users/${createdLikerUser._id}/json`)).body
